// Generated by tsbindgen - Architecture
// Namespace: System.IO
// Facade - Public API Surface

// Import internal declarations
import * as Internal from './internal/index.js';

// Cross-namespace type imports for constraints
import type { SafeFileHandle } from '../Microsoft.Win32.SafeHandles/internal/index.js';
import type { IAsyncEnumerable_1, IEnumerable_1, IReadOnlyList_1 } from '../System.Collections.Generic/internal/index.js';
import type { Collection_1 } from '../System.Collections.ObjectModel/internal/index.js';
import type { IDictionary } from '../System.Collections/internal/index.js';
import type { Component, IComponent, IContainer, ISite, ISupportInitialize, ISynchronizeInvoke } from '../System.ComponentModel/internal/index.js';
import type { MethodBase, MethodInfo } from '../System.Reflection/internal/index.js';
import type { SafeBuffer } from '../System.Runtime.InteropServices/internal/index.js';
import type { ISerializable, SerializationInfo, StreamingContext } from '../System.Runtime.Serialization/internal/index.js';
import type { AccessControlSections, DirectorySecurity, FileSecurity, FileSystemRights } from '../System.Security.AccessControl/internal/index.js';
import type { Encoding, StringBuilder } from '../System.Text/internal/index.js';
import type { Task, Task_1, ValueTask, ValueTask_1 } from '../System.Threading.Tasks/internal/index.js';
import type { CancellationToken } from '../System.Threading/internal/index.js';
import type { ArraySegment_1, AsyncCallback, Boolean as ClrBoolean, Byte, Char, DateTime, Decimal, Delegate, Double, Enum, EventArgs, Exception, Half, IAsyncDisposable, IAsyncResult, ICloneable, IComparable, IConvertible, IDisposable, IFormatProvider, IFormattable, Int16, Int32, Int64, IntPtr, ISpanFormattable, MarshalByRefObject, Memory_1, MulticastDelegate, Nullable_1, Object as ClrObject, ReadOnlyMemory_1, ReadOnlySpan_1, SByte, Single, Span_1, String as ClrString, SystemException, TimeSpan, Type, TypeCode, UInt16, UInt32, UInt64, ValueType, Void } from '../System/internal/index.js';

export * from './internal/index.js';

// Individual type exports for convenience
export { FileSystemAclExtensions$instance as FileSystemAclExtensions } from './internal/index.js';
export { DriveInfo as DriveInfo } from './internal/index.js';
export { DriveNotFoundException as DriveNotFoundException } from './internal/index.js';
export { DriveType as DriveType } from './internal/index.js';
export { ErrorEventArgs as ErrorEventArgs } from './internal/index.js';
export type ErrorEventHandler = Internal.ErrorEventHandler;
export { FileSystemEventArgs as FileSystemEventArgs } from './internal/index.js';
export type FileSystemEventHandler = Internal.FileSystemEventHandler;
export { FileSystemWatcher as FileSystemWatcher } from './internal/index.js';
export { InternalBufferOverflowException as InternalBufferOverflowException } from './internal/index.js';
export { NotifyFilters as NotifyFilters } from './internal/index.js';
export { RenamedEventArgs as RenamedEventArgs } from './internal/index.js';
export type RenamedEventHandler = Internal.RenamedEventHandler;
export { WatcherChangeTypes as WatcherChangeTypes } from './internal/index.js';
export { WaitForChangedResult as WaitForChangedResult } from './internal/index.js';
export { FileLoadException as FileLoadException } from './internal/index.js';
export { FileNotFoundException as FileNotFoundException } from './internal/index.js';
export { Stream as Stream } from './internal/index.js';
export { BinaryReader as BinaryReader } from './internal/index.js';
export { BinaryWriter as BinaryWriter } from './internal/index.js';
export { BufferedStream as BufferedStream } from './internal/index.js';
export { Directory$instance as Directory } from './internal/index.js';
export { DirectoryInfo as DirectoryInfo } from './internal/index.js';
export { DirectoryNotFoundException as DirectoryNotFoundException } from './internal/index.js';
export { EnumerationOptions as EnumerationOptions } from './internal/index.js';
export { EndOfStreamException as EndOfStreamException } from './internal/index.js';
export { File$instance as File } from './internal/index.js';
export { FileAccess as FileAccess } from './internal/index.js';
export { FileAttributes as FileAttributes } from './internal/index.js';
export { FileInfo as FileInfo } from './internal/index.js';
export { FileMode as FileMode } from './internal/index.js';
export { FileOptions as FileOptions } from './internal/index.js';
export { FileShare as FileShare } from './internal/index.js';
export { FileStream as FileStream } from './internal/index.js';
export { FileStreamOptions as FileStreamOptions } from './internal/index.js';
export { FileSystemInfo as FileSystemInfo } from './internal/index.js';
export { HandleInheritability as HandleInheritability } from './internal/index.js';
export { InvalidDataException as InvalidDataException } from './internal/index.js';
export { IOException as IOException } from './internal/index.js';
export { MatchCasing as MatchCasing } from './internal/index.js';
export { MatchType as MatchType } from './internal/index.js';
export { MemoryStream as MemoryStream } from './internal/index.js';
export { Path$instance as Path } from './internal/index.js';
export { PathTooLongException as PathTooLongException } from './internal/index.js';
export { RandomAccess$instance as RandomAccess } from './internal/index.js';
export { SearchOption as SearchOption } from './internal/index.js';
export { SeekOrigin as SeekOrigin } from './internal/index.js';
export { StreamReader as StreamReader } from './internal/index.js';
export { StreamWriter as StreamWriter } from './internal/index.js';
export { StringReader as StringReader } from './internal/index.js';
export { StringWriter as StringWriter } from './internal/index.js';
export { TextReader as TextReader } from './internal/index.js';
export { TextWriter as TextWriter } from './internal/index.js';
export { UnixFileMode as UnixFileMode } from './internal/index.js';
export { UnmanagedMemoryAccessor as UnmanagedMemoryAccessor } from './internal/index.js';
export { UnmanagedMemoryStream as UnmanagedMemoryStream } from './internal/index.js';
